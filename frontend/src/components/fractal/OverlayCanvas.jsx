...
export function OverlayCanvas({ data, matchIndex, width = 980, height = 360 }) {
  ...
  useEffect(() => {
    ...
    // Distribution fan (NEW - using series data)
    if (ds?.p10?.length && ds?.p90?.length) {
      drawDistributionFan(ctx, payload, xAt, yAt, M, plotH);
    }
    ...
    ctx.fillText("— Current", M.l, height - 10);
    ctx.fillText("--- History + Aftermath", M.l + 80, height - 10);
    ctx.fillText("░ P10-P90", M.l + 230, height - 10);
    ctx.fillText("▓ P25-P75", M.l + 310, height - 10);
    ctx.fillText("· · P50", M.l + 390, height - 10);
